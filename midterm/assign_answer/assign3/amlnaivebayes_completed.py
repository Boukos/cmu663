            ...
            if pos == 1:
                ...
                for x in range(len(featlist)):
                    if row[x] in featuredict[featlist[x]]:
                        ...
                        countdict[conc(featlist[x],row[x],localclassval)] = 1   
    featuredict['humidity'].append('low')                        
...
## Modify this function to use the value of smoothing
    ...
    if not(featval in featuredict[feat]):
        ...
        if smoothing > 0:
            total += 1
            val = 1
#    print "*****" + classval + "\t" + feat + "\t" + featval + "\t" + str(val) + "\t" + str(total) + "\n"
...
class Weather(Pmf):
    ...
    def Likelihood(self, data, hypo):
        ...
        for x in range(len(featlist)):
            if featlist[x] != classval:
                like *= condprob(hypo, featlist[x], data[x]) # this is the conditional probability of
 ...
 #               print "%%" + hypo + "\t" + featlist[x] + "\t" + data[x] + "\t" + str(like) + "\n"
        return like
