...
f = open('tblPageCopyEvents.sql','r')  # open up the rblPageCopyEvents table file
pos = 0
for line in f:  # iterate through the entries in the file, skipping over the initial line, which just has the column names
    if pos > 0:
        elts = re.split('[^\w]',line)  # split the line at white space
...
pos = 0
for line in f: # iterate through the entries in the file, skipping over the intial line
    if pos > 0:
...
googleaccessdict = defaultdict(int) # this is used to store the number of google
...
with open('output.csv', 'w') as o:   # open up the output file
    writer = csv.writer(o)           # create a python CSV writer

    # write the header row that lists the columns that will be in the file
    writer.writerow(["LogID", "NumPages", "WikiAccesses", "GoogleAccess", "NumWords"])

    for log in pageaccessdict:  # iterate through the log entries (by their 'log' key).
        ...
        # print out the google access log
        if log in googleaccessdict:
            google_access = googleaccessdict[log]
        else:
            google_access = 0
        ...
        row = [log, page_accesses, wiki_accesses, google_access, words_copied]
